...
XST_UART mySerial(UART0);
#endif

#define SERIAL_WRITE(...)   UART.sendc(__VA_ARGS__)
...
void Adafruit_Fingerprint::begin(uint32_t baudrate) {
	delaymilis(1000); //delay one second to let the sensor 'boot up'	
	UART.begin(baudrate);
...
uint8_t Adafruit_Fingerprint::getStructuredPacket(Adafruit_Fingerprint_Packet * packet, uint16_t timeout) {
  ...
  while(true) {
	//jika tidak ada data serial masuk
    while(UART.RX_buf_full()) { //***
			//jika rx register penuh -> tidak ada data masuk
			//aslinya: !Serial.available()
		  //delay(1);
			small_delay();
      timer++; 
      if( timer >= timeout) {
#ifdef FINGERPRINT_DEBUG
				mySerial.println("Timed out");
#endif
