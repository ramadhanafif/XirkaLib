...
#define wait_us(v) delaymicros(v)
 ...
 * @retval
 */
Enc28J60Network::Enc28J60Network(void):
    MemoryPool()
{ }
...
void Enc28J60Network::init(uint8_t* macaddr) {
    ...
    _spi.settings(true, 0, 3); //master, mode 0, clk div mode 3
    _spi.core_enable();
	
    delay(1000);  // 1 second for stable state
...
uint8_t Enc28J60Network::readByte(uint16_t addr) {
    ...
    // issue read command
    _spi.transfer(ENC28J60_READ_BUF_MEM);
...
void Enc28J60Network::writeByte(uint16_t addr, uint8_t data) {
    ...
    // issue write command
    _spi.transfer(ENC28J60_WRITE_BUF_MEM);
...
uint8_t Enc28J60Network::readOp(uint8_t op, uint8_t address) {
    ...
    // issue read command
    _spi.transfer(op | (address & ADDR_MASK));
...
uint16_t Enc28J60Network::chksum(uint16_t sum, memhandle handle, memaddress pos, uint16_t len) {
    ...
    // issue read command
    spdr = _spi.transfer(ENC28J60_READ_BUF_MEM);
    for (i = 0; i < len; i += 2) {

        // read data
        spdr = _spi.transfer(0x00);
        t = spdr << 8;
        spdr = _spi.transfer(0x00);
        t += spdr;
        sum += t;
