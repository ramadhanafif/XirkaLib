...
class XST_UART {
  public:
    ...
    void print(uint8_t c, uint8_t type = DEC); //print a byte in 2 forms, HEX, DEC

	
		void println(char* string); //append a NL (new line) in end
    void println(int num); //print number as a string, append NL in end.
		//void println(uint8_t c, uint8_t type = DEC); { //print a byte in 3 forms, HEX, DEC append NL in end.
